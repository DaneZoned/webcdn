...
Point.prototype.update = function (delta) {
    ...
    if (mouse.down) {
        ...
        } else if (dist < config.mouse_cut) this.constraints = [];
    }
...
var Cloth = function () {
    ...
    var start_x = canvas.width / 2 - config.cloth_width * config.spacing / 2;
...
window.onload = function () {
    ...
    f1.open();
    //f1.add(config, 'physics_accuracy', 1, 3).name('Physics Accuracy');
    f1.add(config, 'mouse_influence', 1, 30).name('Mouse Influence');
    f1.add(config, 'mouse_cut', 1, 10).name('Cut Size');
    f1.add(config, 'gravity', 0, 1500).name('Gravity');
