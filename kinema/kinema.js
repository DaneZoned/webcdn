$(function () {
    ...
    Tentacle.prototype = {
        ...
        draw: function (ctx) {
            ...
            s = settings.colour.s * 50 * this.shade;
    ...
    var sketch = Sketch.create({
        ...
        update: function () {
            ...
            if (settings.pulse) {
                ...
                radius = settings.headRadius * 0.3 + settings.headRadius * 0.3 * pulse;
            }
        ...
        draw: function () {
            ...
            this.lineWidth = settings.headRadius * 0.1;
    ...
    function onThemeChanged(dark) {
        ...
        settings.colour.h = 30;
        settings.colour.s = 40;
        settings.colour.v = dark ? 0.4 : 0.1;
    ...
    gui.add(settings, 'tentacles').min(1).max(100).name('Hair Count').onChange(onSettingsChanged);
    gui.add(settings, 'thickness').min(1.0).max(40.0).name('Thickness').onChange(onSettingsChanged);
    gui.add(settings, 'length').min(10.0).max(100.0).name('Length').onChange(onSettingsChanged);
    gui.add(settings, 'gravity').min(-3.0).max(3.0).name('Gravity').onChange(onSettingsChanged);
    gui.add(settings, 'wind').min(-3.0).max(3.0).name('Wind').onChange(onSettingsChanged);
    gui.add(settings, 'friction').min(0.0).max(1.0).name('Friction').onChange(onSettingsChanged);

    //var colourGUI = gui.addColor(settings, 'colour');
    //gui.add(settings, 'darkTheme').onChange(onThemeChanged);
    //gui.add(settings, 'pulse');

    var interactiveGUI = gui.add(settings, 'interactive').name('Interactive');
    gui.add(sketch, 'autoclear').name('Clear Shadow');
    gui.add(sketch, 'save').name('Snapshot');
    gui.open();
