...
image.src = 'http://wallpapercave.com/wp/4OqrUKM.jpg';

kaleidoscope = new Kaleidoscope({
    image: image,
    slices: 20
});
kaleidoscope.domElement.style.width = '600px';
kaleidoscope.domElement.style.height = '600px';
kaleidoscope.domElement.style.border = "none";
...
gui.add(kaleidoscope, 'zoom').min(0.25).max(2.0).name('Zoom');

gui.add(kaleidoscope, 'slices').min(6).max(32).step(2).name('Slices');

//gui.add(kaleidoscope, 'radius').min(200).max(500).name('Radius');

gui.add(kaleidoscope, 'offsetX').min(-kaleidoscope.radius).max(kaleidoscope.radius).name('Offset X').listen();

gui.add(kaleidoscope, 'offsetY').min(-kaleidoscope.radius).max(kaleidoscope.radius).name('Offset Y').listen();

gui.add(kaleidoscope, 'offsetRotation').min(-Math.PI).max(Math.PI).name('Offset Rotation').listen();

gui.add(kaleidoscope, 'offsetScale').min(0.5).max(4.0).name('Offset Scale');
