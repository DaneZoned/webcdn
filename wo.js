var config = {
  ...
  max_speed: 2, //pixels
  move_distance: 150, //pixels
  chance_to_move: 20, //percent
...
window.onload = function(){
  ...
  var f1 = gui.addFolder("World lines");
  f1.open();
  f1.add(config, 'max_speed', 1, 5);
  f1.add(config.points, 'max_number', 1, 100);
  f1.add(config, 'chance_to_move', 1, 100);
  f1.add(config, 'chance_to_split', 1, 100);
  f1.add(config, 'move_distance', 1, w);
  f1.add(config, 'split_mode', {"2-Ways": 0, "Side": 1});
  var f2 = gui.addFolder("Eyecandy");
  f2.open();
  var draw_mode = f2.add(config, 'draw_mode', {"Fast": 0, "Fancy": 1});
  var color_mutation;
  draw_mode.onChange(function(){
    if(config.draw_mode === "1")
     color_mutation = f2.add(config, 'color_mutation', 0, 100);
    else
     f2.remove(color_mutation);
  });
    create();
};
